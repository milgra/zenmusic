...
void    vh_textinput_scroller_on_text(view_t* view);
void    vh_textinput_on_button_down(void* userdata, void* data);
                               ...
                               void*       userdata)
{
  ...
  view_t* clear_btn_v  = view->views->data[2];
  cb_t*   cb_btn_press = cb_new(vh_textinput_on_button_down, view);

  vh_button_add(clear_btn_v, VH_BUTTON_NORMAL, cb_btn_press);
  vh_textinput_add(input_txt_v, text, phtext, textstyle, NULL);
  vh_textinput_set_on_text(input_txt_v, vh_textinput_scroller_on_text);
  vh_list_add(list_v, ((vh_list_inset_t){0}), vh_textinput_scroller_item_for_index, NULL, NULL);
...
void vh_textinput_on_button_down(void* userdata, void* btndata)
{
  view_t*                  view = userdata;
  vh_textinput_scroller_t* data = view->handler_data;
