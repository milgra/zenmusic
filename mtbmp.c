...
mtbmp_t*
mtbmp_alloc(int the_width, int the_height);
void
mtbmp_dealloc(void* the_bitmap);
void
mtbmp_clear(mtbmp_t* bitmap);
mtbmp_t*
mtbmp_alloc_from_bitmap(mtbmp_t* the_bitmap);
mtbmp_t*
mtbmp_alloc_from_grayscale(int the_width,
                           int the_height,
                           uint32_t backcolor,
                           uint32_t fontcolor,
                           unsigned char* the_bitmap);
mtbmp_t*
mtbmp_fill_with_color(mtbmp_t* bitmap,
                      int the_sx,
                      int the_sy,
                      int the_ex,
                      int the_ey,
                      uint32_t color);
void
mtbmp_replace_from(mtbmp_t* the_base,
                   mtbmp_t* the_bitmap,
                   int the_x,
                   int the_y);
void
mtbmp_blend_from(mtbmp_t* the_base, mtbmp_t* the_bitmap, int the_x, int the_y);
...
mtbmp_alloc(int the_width, int the_height)
{
  ...
  bitmap->bytes =
    mtmem_calloc(bitmap->bytes_length * sizeof(unsigned char), NULL);
...
void
mtbmp_dealloc(void* pointer)
{
  mtbmp_t* the_bitmap = pointer;
  if (the_bitmap->bytes != NULL)
    mtmem_release(the_bitmap->bytes);
}

void
mtbmp_clear(mtbmp_t* bmp)
{

  memset(bmp->bytes, 0, bmp->bytes_length);
}
                      ...
                      uint32_t color)
{
  ...
  if (the_sy < 0)
    the_sy = 0;
  if (the_ex >= bitmap->width)
    the_ex = bitmap->width;
  if (the_ey >= bitmap->height)
    the_ey = bitmap->height;
...
void
mtbmp_replace_from(mtbmp_t* the_base, mtbmp_t* the_bitmap, int the_x, int the_y)
{
  if (the_x < 0)
    the_x = 0;
  if (the_y < 0)
  ...
  if (the_x + width > the_base->width)
    return; // width = the_base->width - the_x;
  if (the_y + height > the_base->height)
    return; // height = the_base->height - the_y;
...
mtbmp_blend_from(mtbmp_t* the_base, mtbmp_t* the_bitmap, int the_x, int the_y)
{
  ...
  if (bx > the_base->width)
    bx = the_base->width;
  int by = the_y + the_bitmap->height;
  if (by > the_base->height)
    by = the_base->height;
