...
mtpipe_t*
mtpipe_alloc(uint32_t size);
void
mtpipe_dealloc(void* pointer);
char
mtpipe_send(mtpipe_t* boxes, void* data);
void*
mtpipe_recv(mtpipe_t* boxes);
void
mtpipe_test(void);
...
mtpipe_send(mtpipe_t* boxes, void* data)
{
  ...
  if (boxes->flags[boxes->write_index] == 0) {
    ...
    // increment write index, doesn't have to be atomic, this thread uses it
    // only

    boxes->write_index += 1;
    if (boxes->write_index == boxes->size)
      boxes->write_index = 0;
...
mtpipe_recv(mtpipe_t* boxes)
{
  ...
  if (boxes->flags[boxes->read_index] == 1) {
    ...
    boxes->read_index += 1;
    if (boxes->read_index == boxes->size)
      boxes->read_index = 0;
