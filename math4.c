	...
	m4_t m4_defaultrotation( float x , float y , float z )
	{
		...
		if ( max == 0.0 ) return m4_defaultidentity();
	...
	v3_t m4_screen_to_world_coords( m4_t mvpmatrix , v4_t scrvector , float width , float height )
	{
		...
		if ( result.w == 0 ) return v3_init( FLT_MAX, FLT_MAX, FLT_MAX );
	...
	v3_t v4_quadrelativecoors( v4_t ulc , v4_t urc , v4_t llc , v3_t point_is )
	{
		...
		else return v3_init( FLT_MAX , FLT_MAX , FLT_MAX );
	}
	...
	v3_t v4_quadlineintersection( v4_t ulc , v4_t urc , v4_t llc , v3_t linea , v3_t lineb )
	{
		...
		if ( x > 0.0 && x < v3_length( mtvec_ad ) && y < 0.0 && y > -v3_length( mtvec_ab ) ) return point_is;
		else return v3_init( FLT_MAX , FLT_MAX , FLT_MAX );
	}
